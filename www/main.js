...
let profileCroppie = null;
let cropperContext = 'login'; // 'login' | 'profile'

function openGlobalCropper(imgUrl) {
  document.getElementById('cropper-modal').classList.add('open');
  const cropEl = document.getElementById('global-cropper');
  
  if (profileCroppie) {
    profileCroppie.destroy();
  }
  
  profileCroppie = new Croppie(cropEl, {
    viewport: { width: 220, height: 220, type: 'circle' },
    boundary: { width: '100%', height: 280 },
    showZoomer: true
  });
  
  // setTimeout prevents rendering glitches when modal becomes visible
  setTimeout(() => {
    profileCroppie.bind({ url: imgUrl });
  }, 100);
}
...
function confirmProfileCrop() {
  if (!profileCroppie) return;
  profileCroppie.result({ type: 'base64', size: 'viewport', format: 'jpeg' }).then(base64 => {
    user = user || {};
    user.photoData = base64;
    
    if (cropperContext === 'login') {
      const preview = document.getElementById('profile-photo-preview');
      preview.innerHTML = `<img class="photo-preview" src="${base64}" />
        <p style="font-size:12px;color:var(--sage)">Looking sharp! ✓</p>`;
    } else {
      localStorage.setItem('dressly-user', JSON.stringify(user));
      document.getElementById('profile-avatar-display').innerHTML = `<img src="${base64}">`;
      document.getElementById('nav-avatar').innerHTML = `<img src="${base64}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
      showPhotoAnalysis();
...
function renderWardrobe() {
  ...
  const categoriesToRender = categoryOrder.slice();
    ...
    if (grouped[cat] && grouped[cat].length > 0) {
      grouped[cat].forEach(item => {
        ...
        const imgContent = item.photoData
          ? `<img src="${item.photoData}" alt="${item.name}" draggable="false">`
          : item.emoji;
      ...
      html += `<div style="width:100%; text-align:center; color:var(--muted); font-size:13px; padding:24px; border:1px dashed var(--border); border-radius:8px; margin-bottom:8px;">Empty. Drag items here!</div>`;
    }
    html += `</div></div>`;
  });
...
// ======== ADD / EDIT MODAL ========
function populateCategoryDropdown(preselect = 'Shirt') {
  ...
  if (categoryOrder.includes(preselect)) {
    select.value = preselect;
  } else if (categoryOrder.length > 0) {
    select.value = categoryOrder[0];
  }
}
...
function openModal(idx) {
  ...
  if (typeof idx === 'number') {
    ...
    document.getElementById('processing-indicator').style.display = 'none';
    renderEmojiGrid(document.getElementById('item-category').value);
  }
  document.getElementById('processing-indicator').style.display = 'none';
...
function closeCategoryModalIfBg(e) { if (e.target.id === 'category-modal') closeCategoryModal(); }

function saveCategoryModal() {
  const val = document.getElementById('new-category-name').value.trim();
  if (!val) { showToast('Enter a category name'); return; }
  if (categoryOrder.includes(val)) { showToast('Category already exists'); return; }
...
function renderEmojiGrid(cat) {
  const emojis = EMOJIS[cat] || EMOJIS['Shirt'];
...
document.addEventListener('dragstart', e => {
  const t = e.target;
  const handle = t.closest ? t.closest('.drag-handle') : null;
  const card = t.closest ? t.closest('.clothing-card') : null;

  if (handle || (t.classList && t.classList.contains('category-block'))) {
